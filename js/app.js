class RentTaxApp {
    ...
    setInputSteps() {
        ...
        // Поля в объектах с шагом 1000
        const objectStep1000 = ['objectRent'];
        objectStep1000.forEach(id => {
            const element = document.getElementById(id);
            if (element) {
                element.step = '1000';
                element.min = '0';
        ...
        // Поля в договорах с шагом 1000
        const contractStep1000 = ['contractAmount'];
        contractStep1000.forEach(id => {
            const element = document.getElementById(id);
            if (element) {
                element.step = '1000';
                element.min = '0';
    ...
    addInputValidation(inputElement) {
        ...
        inputElement.addEventListener('blur', (e) => {
            ...
            // Запускаем пересчет для полей калькулятора
            if (['rentIncome', 'additionalIncome', 'mortgageExpense', 'utilitiesExpense', 'maintenanceExpense', 'otherExpenses'].includes(e.target.id)) {
                setTimeout(() => {
        ...
        inputElement.addEventListener('input', (e) => {
            ...
            // Запускаем расчет для полей калькулятора
            if (['rentIncome', 'additionalIncome', 'mortgageExpense', 'utilitiesExpense', 'maintenanceExpense', 'otherExpenses'].includes(e.target.id)) {
    ...
    toggleTheme() {
        ...
        // Полностью очищаем классы тем и устанавливаем нужный
    ...
    loadTheme() {
        ...
        if (savedTheme === 'dark') {
            ...
            themeToggle.textContent = '🌙 Тёмная';
            // Убедимся, что тема по умолчанию сохраняется
            if (!savedTheme) {
                localStorage.setItem('theme', 'light');
            }
        }
    ...
    async saveObject(event) {
        ...
        const objectData = {
            ...
            baseRentRate: parseFloat(document.getElementById('objectRent').value) || 0
        };
    ...
    async showContractModal(contractId = null) {
        ...
        if (objects.length === 0) {
            ...
            // Добавляем обработчик изменения выбора объекта
            objectSelect.addEventListener('change', (e) => {
                this.onContractObjectChange(e.target.value, objects);
            });
        }
    ...
    // Обработчик изменения выбора объекта в форме договора
    onContractObjectChange(objectId, objects) {
        if (!objectId) return;

        const selectedObject = objects.find(obj => obj.id == objectId);
        if (selectedObject && selectedObject.baseRentRate) {
            const amountInput = document.getElementById('contractAmount');
            // Подставляем базовую ставку только если поле пустое или равно 0
            if (!amountInput.value || amountInput.value === '0') {
                amountInput.value = selectedObject.baseRentRate;
            }
        }
    }
    ...
    hideContractModal() {
        ...
        // Удаляем обработчик изменения объекта при закрытии модального окна
        const objectSelect = document.getElementById('contractObject');
        objectSelect.removeEventListener('change', this.onContractObjectChange);
    }
    ...
    async saveContract(event) {
        ...
        const contractData = {
            ...
            rentAmount: parseFloat(document.getElementById('contractAmount').value) || 0,
