class RentTaxApp {
    ...
    addInputValidation(inputElement) {
        // Устанавливаем начальное значение 0, но только если поле пустое
        if (inputElement.value === '') {
            inputElement.value = '0';
        }

        // Обработчик фокуса - очищаем поле если значение "0"
        ...
        // Обработчик потери фокуса - если пусто, ставим "0"
        inputElement.addEventListener('blur', (e) => {
            if (e.target.value === '') {
                e.target.value = '0';
            } else {
                // Убираем ведущие нули только если число > 0
        ...
        inputElement.addEventListener('paste', (e) => {
            ...
            if (numbersOnly) {
                // Просто заменяем текущее значение
